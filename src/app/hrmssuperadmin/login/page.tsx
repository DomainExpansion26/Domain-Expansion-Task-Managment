...
import { Building2, Mail, Lock, ArrowRight, AlertCircle, CheckCircle2, UserPlus, Eye, EyeOff } from "lucide-react";
...
function HRMSSuperAdminLoginForm() {
  ...
  useEffect(() => {
    fetch("/api/auth/me")
      .then((res) => res.json())
      .then((json) => {
        if (json.success && (json.data?.user?.role === "HR_ADMIN" || json.data?.user?.role === "SUPER_ADMIN")) {
          router.replace("/hrmssuperadmin");
        } else {
