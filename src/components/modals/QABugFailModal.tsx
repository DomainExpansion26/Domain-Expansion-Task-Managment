...
export function QABugFailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg bg-[#141414] border border-red-500/40 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2E2E2E] bg-red-950/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-base font-black text-white">{bugKey} &bull; Fail Defect Test</h2>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="p-3 rounded-xl bg-[#1A1A1A] border border-[#2E2E2E] text-xs">
            <div className="text-[10px] font-mono text-[#888898] uppercase">Defect Summary</div>
            <div className="text-white font-semibold truncate mt-0.5">{bugTitle}</div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-[#1A1A1A] border border-[#2E2E2E] text-white text-xs placeholder-[#666] focus:border-red-500 focus:outline-none"
            />
          </div>

          {/* Actual Result observed during testing */}
          <div>
            <label className="block text-xs font-bold text-[#ACACB8] mb-1.5">Actual Result Observed</label>
            <textarea
              rows={2}
              value={actualResult}
              onChange={(e) => setActualResult(e.target.value)}
              placeholder="e.g. Server returned 500 Internal Server Error with unhandled PrismaUniqueConstraintViolation"
              className="w-full px-3.5 py-2 rounded-xl bg-[#1A1A1A] border border-[#2E2E2E] text-white text-xs placeholder-[#666] focus:border-red-500 focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2 rounded-xl bg-[#1A1A1A] border border-[#2E2E2E] text-white text-xs placeholder-[#666] focus:border-red-500 focus:outline-none font-mono text-[11px]"
            />
          </div>
          ...
          {/* Footer Actions */}
          <div className="pt-3 border-t border-[#2E2E2E] flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-xl bg-[#222] hover:bg-[#2A2A2A] text-[#ACACB8] hover:text-white text-xs font-semibold"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="flex items-center gap-2 px-5 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-bold shadow-lg shadow-red-600/20 disabled:opacity-50"
            >
